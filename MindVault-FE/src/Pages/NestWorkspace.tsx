                    ...
                    <div className="flex items-center gap-3">
                         ...
                         <div className="flex bg-slate-800 p-1 rounded-lg border border-slate-700">
                             <button 
                                ...
                                className={`p-2 rounded-md transition-all ${viewMode === 'gallery' ? 'bg-slate-700 shadow-sm text-indigo-400' : 'text-slate-400 hover:text-slate-300'}`}
                                title="Gallery View"
                             >
                                 <LayoutGrid size={18} />
                             </button>
                             <button 
                                ...
                                className={`p-2 rounded-md transition-all ${viewMode === 'chat' ? 'bg-slate-700 shadow-sm text-indigo-400' : 'text-slate-400 hover:text-slate-300'}`}
                ...
                <main className="flex-1 overflow-hidden relative">
                    ...
                    {viewMode === 'gallery' ? (
                        ...
                        <div className="flex h-full">
                            ...
                            <div className="w-80 border-r border-slate-800 bg-slate-900/30 overflow-y-auto hidden md:block">
                                <div className="p-4 sticky top-0 bg-inherit z-10 border-b border-slate-800">
                                    <div className="relative">
                                        <Search size={14} className="absolute left-3 top-3 text-slate-500" />
                                        <input 
                                            type="text" 
                                            placeholder="Search context..." 
                                            className="w-full pl-9 pr-4 py-2 text-sm bg-slate-800 border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/50 text-slate-200 placeholder-slate-500"
                                        />
                                    </div>
                                </div>
                                <div className="p-2 space-y-1">
                                    {nestLinks.map((link) => (
                                        <div key={link._id} className="flex items-start gap-3 p-3 hover:bg-slate-800 rounded-lg cursor-pointer transition-colors border border-transparent hover:border-slate-700 group">
                                            <div className="mt-0.5 text-lg opacity-70 group-hover:opacity-100">📁</div>
                                            <div className="overflow-hidden">
                                                <h4 className="text-sm font-medium text-slate-200 truncate">{link.title}</h4>
                                                <p className="text-xs text-slate-500 truncate">{link.link}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="p-4 border-t border-slate-800 bg-slate-950/80 backdrop-blur">
                                    <div className="max-w-3xl mx-auto relative group">
                                        <input 
                                            ...
                                            className="w-full pl-4 pr-12 py-3.5 bg-slate-900 border border-slate-700 rounded-xl focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all text-white placeholder-slate-500"
                                        />
                                        <button 
                                            onClick={handleSend}
                                    ...
                                    <p className="text-center text-[10px] text-slate-500 mt-2">AI Analyst can make mistakes. Check key takeaways.</p>
                                </div>
                            </div>
                        </div>
                    )}
